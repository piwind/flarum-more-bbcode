    ...
    public function __invoke(Configurator $config)
    {
        ...
        if ($this->functions->isEnable("reply")) {
            $config->BBCodes->addCustom(
                ...
                $this->translator->trans('piwind-more-bbcode.forum.xx2see.reply.title')
                . '</div><xsl:apply-templates /></div>'
            );
            $config->tags->add("reply2see_alert")->template =
                '<div class="xx2see"><div class="xx2see_alert">' .
                $this->translator->trans(
                    'piwind-more-bbcode.forum.xx2see.reply.deny',
                    array(
        ...
        if ($this->functions->isEnable("login"))
            $config->BBCodes->addCustom(
                ...
                $this->translator->trans('piwind-more-bbcode.forum.xx2see.login.title') .
                '</div><xsl:apply-templates /></div>'
            );
        if ($this->functions->isEnable("login"))
            $config->tags->add("login2see_guest")->template =
                ...
                $this->translator->trans(
                    'piwind-more-bbcode.forum.xx2see.login.deny',
                    array(
        ...
        if ($this->functions->isEnable("like")) {
            $config->BBCodes->addCustom(
                ...
                $this->translator->trans('piwind-more-bbcode.forum.xx2see.like.title')
                . '</div><xsl:apply-templates /></div>'
            );
            $config->tags->add("like2see_alert")->template =
                '<div class="xx2see like"><div class="xx2see_alert">' .
                $this->translator->trans(
                    'piwind-more-bbcode.forum.xx2see.like.deny',
